...
//        case .production:
            unitsParams = [
                (.Quiz,.playlist(tags: [], channelToken: "64d0ae93dc9c1f0012f5cf23", context: false, fallback: true))
...
//        self.unitsParams = unitsParams
        return makeUnitConfigurations(with: unitsParams, environment: environment, hideApesterAds: hideApesterAds, gdprString: gdprString, baseUrl: UnitConfigurationsFactory.baseUrl)
    }
    ...
    func setEnvironment(env: APEEnvironment)  {
        UnitConfigurationsFactory.environment = env
    }
}
